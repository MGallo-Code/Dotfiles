...
# Regen-CombinedAgentRules during setup and sync. (parity-checked: scripts/ci/check-parity.py)
...
function Regen-CombinedAgentRules {
    ...
    foreach ($file in (Get-ChildItem -Path $GlobalRulesDir -Filter *.md | Sort-Object Name)) {
        [void]$sb.Append((Get-Content -Raw -Path $file.FullName))
        [void]$sb.Append("`n`n")
    }
    $content = $sb.ToString()
    foreach ($target in $CombinedRulesTargets) {
        ...
        if ((Test-Path $target) -and ((Get-Content -Raw -Path $target) -eq $content)) {
            Set-ItemProperty -Path $target -Name IsReadOnly -Value $true -ErrorAction SilentlyContinue
            Write-Ok "combined-rules: $target already current (read-only)"
            continue
        }
        if (Test-Path $target) {
            Copy-Item $target "$target.sync-backup" -Force
            Write-Warn "combined-rules: $target differed from source; backed up to $target.sync-backup"
        }
        if (Test-Path $target) {
            Set-ItemProperty -Path $target -Name IsReadOnly -Value $false -ErrorAction SilentlyContinue
        }
