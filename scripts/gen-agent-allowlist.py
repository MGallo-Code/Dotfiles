...
settings and each project's settings, then translate into each tool's NATIVE mechanism:
  ...
  - gemini (~/.gemini/settings.json): per-MCP-SERVER mcpServers.<s>.trust = true,
                                      plus ~/.gemini/policies/dotfiles-allowlist.toml
                                      for shell/web approvals
...
Idempotent: re-running converges machine config to the current native mechanisms.
Backs up each target to <file>.allowlist-bak before writing. Gemini's deprecated
tools.allowed setting is migrated into Policy Engine TOML, then removed.
"""
import glob
import json
import os
...
def _toml_array(values):
    ...
    if not vals:
        return "[]"
    body = ",\n".join(f"  {_toml_string(v)}" for v in vals)
    return "[\n" + body + ",\n]"
...
# ── 3. gemini: per-server trust + Policy Engine shell/web allowlist ─────────
def _parse_gemini_legacy_allowed(entries):
    bash_prefixes, web = set(), set()
    for entry in entries:
        if not isinstance(entry, str):
            continue
        if entry.startswith("run_shell_command(") and entry.endswith(")"):
            prefix = entry[len("run_shell_command("):-1].strip()
            if prefix:
                bash_prefixes.add(prefix)
        elif entry in {"google_web_search", "web_fetch"}:
            web.add(entry)
    return bash_prefixes, web


def _gemini_policy_content(bash_prefixes, web):
    lines = [
        "# AUTO-GENERATED by dotfiles scripts/gen-agent-allowlist.py.",
        "# Claude permissions are the source of truth; do not hand-edit this file.",
        "",
    ]
    if bash_prefixes:
        lines += [
            ...
            "commandPrefix = " + _toml_array(bash_prefixes),
            'decision = "allow"',
            "priority = 100",
            "",
        ]
    ...
    if len(lines) == 3:
        lines += [
            "# No shell/web approvals mirrored yet.",
            "",
        ]
    return "\n".join(lines)
...
def update_gemini(mcp_tools, mcp_servers, bash_prefixes, web):
    ...
    # Migrate legacy tools.allowed into policy rules so we don't lose manual grants while
    # removing the deprecated setting that Gemini CLI warns about.
    legacy_allowed = []
    tools = d.get("tools")
    if isinstance(tools, dict):
        legacy_allowed = tools.get("allowed", []) or []
        if "allowed" in tools:
            del tools["allowed"]
            changed += 1
        if not tools:
            del d["tools"]
    legacy_bash, legacy_web = _parse_gemini_legacy_allowed(legacy_allowed)
    policy_bash = set(bash_prefixes) | legacy_bash
    policy_web = set(web) | legacy_web

    policy_dir = os.path.join(HOME, ".gemini", "policies")
    policy_path = os.path.join(policy_dir, "dotfiles-allowlist.toml")
    os.makedirs(policy_dir, exist_ok=True)
    policy_content = _gemini_policy_content(policy_bash, policy_web)
    old_policy = ""
    if os.path.exists(policy_path):
        with open(policy_path, encoding="utf-8") as f:
            old_policy = f.read()
    if old_policy != policy_content:
        if old_policy:
            shutil.copy2(policy_path, policy_path + ".allowlist-bak")
        toml = _load_toml()
        if toml is not None:
            toml.loads(policy_content)
    ...
    if changed:
        ...
        return (
            f"gemini: +{changed} change(s) "
            f"(server trust + policy allowlist with {len(policy_bash)} shell prefixes)"
        ), changed
    return "gemini: trust + policy allowlist already current", 0
